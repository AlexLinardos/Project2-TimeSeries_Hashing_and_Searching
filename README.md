...
* To run Clustering you are going to need the following parameters:
  ...
  7. silhouette: if given, prints Silhouette in the output (explained further in the [TimeSeries-Clustering/](timeSeries-clustering/) in-depth analysis)
...
__Examples :__<br>
(*Please keep in mind that copying from here and pasting in the terminal may cause problems.*)
...
__Tested examples with good results :__<br>
(*Please keep in mind that copying from here and pasting in the terminal may cause problems.*)

1. `$./bin/search –i datasets/nasd_input.csv –q datasets/nasd_query.csv –k 1 -L 1 -ο outputs/output.txt -algorithm LSH`
2. `$./bin/search –i datasets/nasd_input.csv –q datasets/nasd_query.csv –k 1 -M 30 -probes 1 -ο outputs/output.txt -algorithm Hypercube`
3. `$./bin/search –i datasets/nasd_input.csv –q datasets/nasd_query.csv -L 6 -ο outputs/output.txt -algorithm Frechet -metric discrete`
4. `$./bin/search –i datasets/nasd_input.csv –q datasets/nasd_query.csv -ο outputs/output.txt -algorithm Frechet -metric continuous`
...
### src/
  ...
  2. __cluster_main.cpp__: Main code file that executes the clustering process according to the command line parameters given by the user. It checks the value of the -update parameter to determine the method used at the assignment step and if it's going to cluster vectors or curves. Then it checks the value of the -assignment parameter to determine the method used for the update step. At the end, it produces the output file with or without the detailed presentation of clusters and the Silhouette metric, according to if the parameters -complete and -silhouette where given respectively.
